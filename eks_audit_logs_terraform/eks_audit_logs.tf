# We're only leveraging the AWS provider such that we are provisioning the resources directly.
provider "aws" {
  region = "us-west-1"
}
...
locals {
  ...
  # If you're using an existing s3 bucket to store audit logs
  ...
  # We have the ability to set custom encryption keys...
  # and use them to encrypt various elements of the setup including the sns topic and the firehose data stream
...
#tfsec:ignore:aws-s3-enable-bucket-loggingß
